...
/* ---- Vercel runtime & max duration ----
   If you're on Vercel Pro, 800s covers ~9-10 min runs.
   Keep the resumable design below so you never time out as the catalog grows. */
   export const runtime = 'nodejs';
   export const maxDuration = 800; // set to 300 if you're not on Pro
   ...
   async function acquireOrValidateLock(runId) {
     // If no lock, create it with our runId. If lock exists, only allow if held by same runId.
   ...
   /* ----------------- tunables ----------------- */
   // Shopify REST default is ~2 rps. 500ms delay is safe and faster than 600ms.
   const MIN_DELAY_MS   = Number(process.env.SHOPIFY_THROTTLE_MS || 500);
   // Keep each slice under ~4.5m to leave headroom inside a 5m/8m function window.
   ...
   /* ----------------- Notification helper ----------------- */
   async function notifyPending({ product, pending, keysTried, pid, title, handle, isBundle }) {
     ...
     await setSubscribersForProduct({ id: pid, handle }, pending);
         ...
         if (isBundle) {
           ...
           if (mf?.value) {
             ...
             for (const c of comps) {
               ...
               // prefer quantity from this product's variants if present; otherwise fetch the variant
           ...
           if (shouldNotify) {
             const counts = await notifyPending({ product, pending, keysTried, pid, title, handle, isBundle: true });
           ...
           if (shouldNotify) {
             const counts = await notifyPending({ product, pending, keysTried, pid, title, handle, isBundle: false });
             notificationsSent    += counts.notificationsSent;
             smsNotificationsSent += counts.smsNotificationsSent;
             notificationErrors   += counts.notificationErrors;
             profileUpdates       += counts.profileUpdates;
   ...
   export async function GET(req) {
     ...
     if (!runId) {
       const cur = await redis.get(CURSOR_KEY);
       runId = cur?.runId || (globalThis.crypto?.randomUUID?.() ? crypto.randomUUID() : `${Date.now()}-${Math.random().toString(16).slice(2)}`);
     }
