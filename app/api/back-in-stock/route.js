...
async function readBody(request) {
  ...
  // Fallback: try json, else parse as text params
  const raw = await request.text();
  try {
...
export async function POST(request) {
  ...
  try {
        ...
        NextResponse.json(
          {
            success: false,
            error:
              "Server misconfigured: missing KLAVIYO_API_KEY or KLAVIYO_LIST_ID",
          },
          { status: 500 }
        ),
        origin
    ...
    // sms consent can come in as "true"/"on"/"1"
    const smsFlag =
      smsAllowed === true ||
      sms_consent === true ||
      smsAllowed === "true" ||
    ...
    // derive names if only full_name exists
    ...
    // Best-effort Redis: store subscribers per product
    ...
    if (redis_ok) {
      ...
      const writes = [
        redis.set(idKey, merged, { ex: 90 * 24 * 60 * 60 }),
      ];
    ...
    return cors(
      NextResponse.json({
        ...
        klaviyo_success,
        klaviyo_status,
        // keep these for debugging; if you don’t want them exposed, remove them
        // klaviyo_body,
    ...
    // IMPORTANT: log the real error so Vercel logs show it
